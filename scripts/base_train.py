...
class BaseTrainConfig:
    ...
    def from_args(cls, args):
        """Create config from CLI args, dict, Namespace, or config object."""
        if args is None:
            return cls()
...
class BaseTrainResults:
    """Results from base model training."""
    status: str
    final_val_bpb: Optional[float] = None
    min_val_bpb: Optional[float] = None
    core_metric: Optional[float] = None
    mfu_percent: Optional[float] = None
    training_time_min: Optional[float] = None
    num_steps: int = 0
    checkpoint_dir: Optional[str] = None

def main(config: Optional[BaseTrainConfig] = None) -> BaseTrainResults:
    """
    Run base model training.

    Args:
        config: Training configuration. None = parse from CLI.

    Returns:
        BaseTrainResults with training metrics.
    """
    # Parse arguments
    if config is None:
        ...
        config = BaseTrainConfig.from_args(args)
    elif not isinstance(config, BaseTrainConfig):
        config = BaseTrainConfig.from_args(config)

    # Convert config to Namespace for backward compatibility with existing code
    args = argparse.Namespace(**asdict(config))
...
if __name__ == "__main__":
    ...
    if results.status == 'success':
        ...
        metrics = []
        if results.final_val_bpb is not None:
            metrics.append(f"val_bpb={results.final_val_bpb:.4f}")
        if results.core_metric is not None:
            metrics.append(f"CORE={results.core_metric:.4f}")
        if results.mfu_percent is not None:
            metrics.append(f"MFU={results.mfu_percent:.2f}%")
        if metrics:
            print0(f"Final metrics: {', '.join(metrics)}")
    else:
        print0(f"\nTraining failed!")
        import sys
        sys.exit(1)
